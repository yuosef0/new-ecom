...
export default function ShippingPage() {
    ...
    useEffect(() => {
        async function load() {
            const data = await getPage('shipping');
            if (data) {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            await updatePage('shipping', page);
                ...
                <div className="flex-1">
                    <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-lg bg-brand-primary/10 flex items-center justify-center">
                            <span className="material-icons-outlined text-brand-primary text-2xl">local_shipping</span>
                        </div>
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Shipping & Delivery Page</h1>
                            <p className="mt-1 text-gray-600">Manage your Shipping & Delivery page content</p>
                        </div>
                    </div>
                </div>
            </div>
