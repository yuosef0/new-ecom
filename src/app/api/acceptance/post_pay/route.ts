...
/**
 * Paymob Default Callback Handler
 * This is the default Paymob callback URL: /api/acceptance/post_pay
 * Paymob redirects here after payment completion
 */
export async function GET(request: NextRequest) {
    try {
        const searchParams = request.nextUrl.searchParams;

        // Extract all Paymob parameters
        ...
        console.log("[Paymob Default Callback] Received:", {
            success,
            orderId,
            transactionId,
            amountCents,
        ...
        if (hmac && process.env.PAYMOB_HMAC_SECRET) {
            const hmacString = [
                ...
                searchParams.get("source_data.pan") || "NA",
            ...
            if (calculatedHMAC !== hmac) {
                console.error("[Paymob Default Callback] Invalid HMAC signature");
        ...
        if (!orderId) {
            console.error("[Paymob Default Callback] Missing order ID");
            return NextResponse.redirect(
        ...
        if (orderError || !order) {
            console.error("[Paymob Default Callback] Order not found:", orderId);
            return NextResponse.redirect(
        ...
        if (success === "true") {
            ...
            if (updateError) {
                console.error("[Paymob Default Callback] Error updating order:", updateError);
            } else {
                console.log("[Paymob Default Callback] Order updated successfully:", order.order_number);
                    ...
                    for (const item of items) {
                        if (item.variant_id) {
                            try {
                                ...
                                console.error("[Paymob Default Callback] Error decrementing stock:", error);
                            }
                        }
                    }
                }
            ...
            console.log("[Paymob Default Callback] Payment failed for order:", order.order_number);
        ...
        console.error("[Paymob Default Callback] Error processing callback:", error);
        return NextResponse.redirect(
